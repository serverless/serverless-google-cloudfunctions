...
describe('GooglePackage', () => {
  ...
  describe('#constructor()', () => {
    ...
    describe('hooks', () => {
      ...
      it('should run "package:cleanup" promise chain', () => googlePackage.hooks['package:cleanup']().then(() => {
        expect(cleanupServerlessDirStub.calledOnce).toEqual(true);
      }));

      it('should run "before:package:initialize" promise chain', () => googlePackage.hooks['before:package:initialize']().then(() => {
        expect(validateStub.calledOnce).toEqual(true);
        expect(setDefaultsStub.calledAfter(validateStub)).toEqual(true);
      }));

      it('should run "package:initialize" promise chain', () => googlePackage.hooks['package:initialize']().then(() => {
      ...
      it('should run "before:package:compileFunctions" promise chain', () => googlePackage.hooks['before:package:compileFunctions']().then(() => {
        expect(generateArtifactDirectoryNameStub.calledOnce).toEqual(true);
      }));

      it('should run "package:compileFunctions" promise chain', () => googlePackage.hooks['package:compileFunctions']().then(() => {
        expect(compileFunctionsStub.calledOnce).toEqual(true);
      }));

      it('should run "package:finalize" promise chain', () => googlePackage.hooks['package:finalize']().then(() => {
